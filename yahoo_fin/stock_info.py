...
def build_url(ticker, start_date = None, end_date = None, interval = "1d", includePrePost = False):
    ...
    params = {"period1": start_seconds, "period2": end_seconds,
              "interval": interval.lower(), "events": "div|split|earn"}
...
def get_data(ticker, start_date = None, end_date = None, index_as_date = True, interval = "1d", includePrePost = False,
    ...
    site, params = build_url(ticker, start_date, end_date, interval, includePrePost)
...
def get_live_price(ticker):
    ...
    '''Gets the live price of input ticker
       ...
       @param: ticker
    '''
    start_date = pd.Timestamp('now', tz='UTC') - pd.Timedelta(days=1)
    end_date = pd.Timestamp('now', tz='UTC') + pd.Timedelta(days=1)

    df = get_data(ticker, start_date = start_date, end_date = end_date, interval = '1m', includePrePost = True)
